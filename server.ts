...
Kembalikan pesan dalam bentuk JSON dengan key "masalah_rekap" and "solusi_rekap". Jangan ada tambahan teks di luar JSON.`;
    ...
    // To ensure maximum compatibility with Google Apps Script (which parses URL parameters or POST JSON body):
    // 1. We keep the request URL neat and compact to completely avoid HTTP 400/414 (Request-URI Too Large) errors from Google's servers.
    // 2. We pass the full robust payload containing all field and casing permutations inside the POST JSON body.
...
Di akhir, tambahkan tag [ACTION:CONTACT_OFFICER] jika masalah tergolong berat (epidemi).`;
    ...
    // Send in both standard data-error format (resilient) and legacy event: error (backward compatible)
