...
import { Plus, Edit, Trash2, X, Info, ShieldAlert, CheckCircle2, Leaf, BookOpen, Layers, Search } from 'lucide-react';
...
const OptInformation: React.FC<OptInformationProps> = ({ userRole }) => {
  ...
  const loadData = async () => {
    try {
      const data = await SupabaseService.fetchRemoteData('katalog_hama');
      setPests(data);
      localStorage.setItem('popt_pests', JSON.stringify(data));
      setSupabaseConnected(true);
    } catch (err: any) {
      console.warn("Supabase fetch failed initially (normal if unconfigured):", err.message);
  ...
  const handleAddPest = async (e: React.FormEvent) => {
    e.preventDefault();
    let updatedPests: PestInfo[];
    let pestToSave: PestInfo;

    if (editingPestId) {
      pestToSave = {
        ...pests.find(p => p.id === editingPestId)!,
        ...newPest as PestInfo
      };
      updatedPests = pests.map(p => 
        p.id === editingPestId ? pestToSave : p
      );
    } else {
      pestToSave = {
        ...newPest as PestInfo,
    ...
    try {
      await SupabaseService.saveRemoteData('katalog_hama', pestToSave);
      setSupabaseConnected(true);
    } catch (err: any) {
  ...
  const handleDeletePest = async (id: string) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus informasi ini?')) {
      ...
      try {
        await SupabaseService.deleteRemoteData('katalog_hama', id);
      } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative">
            <input 
              type="text" 
              placeholder="Cari tanaman atau nama OPT..."
              value={searchTerm}
          ...
          <span className="bg-slate-100 px-3 py-1.5 rounded-full text-xs font-semibold text-slate-500">
      ...
      {userRole === 'Admin' && (
        <SupabaseConfigPanel 
          tableType="katalog_hama"
          onDataSynced={(syncedData) => {
            setPests(syncedData);
            localStorage.setItem('popt_pests', JSON.stringify(syncedData));
            setSupabaseConnected(true);
          }}
          getLocalData={() => {
            const saved = localStorage.getItem('popt_pests');
            return saved ? JSON.parse(saved) : PEST_DATA;
          }}
        />
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredPests.length > 0 ? (
          ...
          <div className="col-span-full py-16 text-center bg-white border border-slate-100 rounded-2xl shadow-sm">
            ...
            <p className="text-slate-500 font-medium text-sm">Hama atau OPT tanaman tersebut tidak ditemukan.</p>
            <p className="text-slate-400 text-xs mt-1">Coba gunakan kata kunci tanaman (padi, jagung, cabai) atau jenis OPT lainnya.</p>
          </div>
        )}
      </div>
