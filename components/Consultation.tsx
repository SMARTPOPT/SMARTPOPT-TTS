...
const Consultation: React.FC<ConsultationProps> = ({ onNavigate }) => {
  ...
  const updateRecordWithConsultation = (question: string, aiResponse: string, image?: string, latestAiMessage?: string) => {
    ...
    if (updatedRecord && !hasSyncedRef.current[currentTicketId]) {
      // Ensure there is a meaningful question/response before sync and robustly wait for a diagnostic conclusion block
      const isComplete = question && 
                         question !== '(Mulai Konsultasi)' && 
                         aiResponse && 
                         aiResponse !== '(Proses)' && 
                         (aiResponse.toUpperCase().includes('DIAGNOSA:') || aiResponse.toUpperCase().includes('DIAGNOSIS:'));
      if (isComplete) {
