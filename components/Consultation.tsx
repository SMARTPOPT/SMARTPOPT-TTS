  ...
  const handleSend = async () => {
    ...
    if (flowStep !== 'CONSULTATION') {
      ...
      if (flowStep === 'NAME') {
        ...
        setMessages(prev => [...prev, { role: 'ai', text: `Terima kasih Pak/Bu ${userMsg}. Selanjutnya, apa Alamat atau Lokasi lahan Bapak/Ibu?` }]);
        setFlowStep('ADDRESS');
        return;
      }
      ...
      if (flowStep === 'CONFIRMATION') {
        if (userMsg.toLowerCase().match(/ya|ok|betul|benar|sesuai|sip/)) {
          ...
          setFlowStep('NAME');
          setFarmerData({ name: '', address: '', group: '' });
        }
        return;
      }
    }
    ...
    try {
      ...
      if (hasAction) {
        const problemMatch = cleanText.match(/(?:RINGKASAN MASALAH|MASALAH UMUM):?\s*([\s\S]*?)(?=DIAGNOSA|$)/i);
        const diagnosisMatch = cleanText.match(/DIAGNOSA:?\s*([\s\S]*?)(?=SARAN|$)/i);
        const adviceMatch = cleanText.match(/SARAN (?:PENGENDALIAN)?:?\s*([\s\S]*)$/i);

        updateRecordWithConsultation(
          problemMatch ? problemMatch[1].trim() : userMsg,
          `DIAGNOSA: ${diagnosisMatch ? diagnosisMatch[1].trim() : '-'}\n\nSARAN: ${adviceMatch ? adviceMatch[1].trim() : '-'}`,
          userImgPreview,
          cleanText
        );
      }
      ...
      if (err.message?.includes('API_KEY')) {
        errorMessage = 'Konfigurasi AI belum siap. Mohon pastikan API Key sudah terpasang.';
      } else if (err.message?.includes('fetch')) {
        errorMessage = 'Koneksi internet terputus. Silakan periksa jaringan Anda dan coba lagi.';
      }
